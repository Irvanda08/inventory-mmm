...
if ($id_barang) {
    ...
    // 2. HITUNG SALDO AWAL (Stok awal master + transaksi sebelum tgl_awal)
    $saldo_awal = $stok_awal_master;
    if ($tgl_awal) {
        $sql_prev = "SELECT jenis, jumlah FROM transaksi_barang WHERE id_barang=$id_barang AND tanggal < '".mysqli_real_escape_string($conn, $tgl_awal)."'";
        $prev_res = mysqli_query($conn, $sql_prev);
        while ($p = mysqli_fetch_assoc($prev_res)) {
            if (strtolower($p['jenis']) == 'masuk') $saldo_awal += $p['jumlah'];
            else $saldo_awal -= $p['jumlah'];
        }
    }

    // 3. Ambil transaksi dalam rentang periode
...
.table-custom th { @apply bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-widest p-4 border-b border-slate-100; }
.table-custom td { @apply p-4 border-b border-slate-50 text-sm text-slate-600; }

/* Pengaturan Print Sangat Rapat */
@media print {
    .no-print, aside, nav, header, form { display: none !important; }
    ...
    .table-custom th, .table-custom td {
        padding: 4px 8px !important; /* Row sangat rapat */
        font-size: 9pt !important;
        border: 1px solid #000 !important;
        line-height: 1 !important;
    }
    .print-header { display: flex !important; justify-content: justify; border-bottom: 2px solid #000; margin-bottom: 10px; padding-bottom: 10px; }
}
</style>
...
<main class="flex-1 bg-slate-50 min-h-screen p-4 md:p-8 w-full">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-200/60 p-6 mb-8 no-print">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <button type="submit" class="bg-slate-800 hover:bg-black text-white px-6 py-2.5 rounded-xl font-black text-sm uppercase tracking-widest transition-all">
                Tampilkan
            </button>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 no-print">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-5">
            ...
            <div>
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Total Masuk</p>
                <p class="text-2xl font-black text-slate-800"><?= number_format($total_masuk); ?> <span class="text-xs text-slate-400 uppercase"><?= $satuan ?></span></p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-5">
            <div class="w-14 h-14 bg-rose-50 text-rose-600 rounded-2xl flex items-center justify-center text-2xl">📤</div>
            <div>
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Total Keluar</p>
                <p class="text-2xl font-black text-slate-800"><?= number_format($total_keluar); ?> <span class="text-xs text-slate-400 uppercase"><?= $satuan ?></span></p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-5">
            <div class="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center text-2xl">📦</div>
            <div>
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest">Stok Akhir</p>
                <p class="text-2xl font-black text-slate-800"><?= number_format($saldo_awal + $total_masuk - $total_keluar); ?> <span class="text-xs text-slate-400 uppercase"><?= $satuan ?></span></p>
            </div>
        </div>
    </div>
    ...
    <div id="printable" class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 p-8">
        <div class="print-header flex justify-between items-end border-b-2 border-slate-800 pb-6 mb-8">
            <div>
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">PT MMM - Manajemen Inventaris Terpadu</p>
            </div>
        ...
        <table class="table-custom">
            <thead>
                <tr>
                    ...
                    <th class="text-center w-28 bg-slate-50">Sisa Stok</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                <tr class="bg-slate-50/50 font-bold">
                    <td class="text-slate-400 text-xs text-center italic" colspan="2">SALDO AWAL PERIODE</td>
                    <td class="text-center">-</td>
                    <td class="text-center">-</td>
                    <td class="text-center text-blue-600 bg-slate-100/50"><?= number_format($saldo_awal) ?></td>
                </tr>

                <?php
                $current_sisa = $saldo_awal;
                if(count($transaksi) > 0):
                    foreach($transaksi as $t):
                        $jenis = strtolower($t['jenis']);
                        $masuk = ($jenis=='masuk') ? $t['jumlah'] : 0;
                        $keluar = ($jenis=='keluar') ? $t['jumlah'] : 0;
                        $current_sisa += $masuk - $keluar;
                ?>
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="font-bold text-slate-600 text-xs"><?= date('d/m/Y', strtotime($t['tanggal'])); ?></td>
                    <td class="text-slate-700"><?= htmlspecialchars($t['keterangan']); ?></td>
                    <td class="text-center text-emerald-600 font-black"><?= $masuk ? '+'.number_format($masuk) : '-' ?></td>
                    <td class="text-center text-rose-600 font-black"><?= $keluar ? '-'.number_format($keluar) : '-' ?></td>
                    <td class="text-center font-black text-slate-800 bg-slate-50/30"><?= number_format($current_sisa) ?></td>
                </tr>
                <?php endforeach; else: ?>
                <tr>
                    <td colspan="5" class="text-center py-20 text-slate-400 italic">Tidak ada pergerakan stok pada periode ini.</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
        ...
        <div class="mt-12 hidden print:grid grid-cols-2 text-center">
            ...
            <div class="flex flex-col items-center">
                <p class="text-[10px] font-bold text-slate-400 uppercase mb-20">Yogyakarta, <?= date('d F Y') ?></p>
                <p class="font-bold text-slate-800 border-t-2 border-slate-800 inline-block px-12 pt-2 tracking-widest text-xs">Kord. Workshop</p>
                <p class="text-[9px] text-slate-400 font-bold uppercase mt-1">PT Muara Mitra Mandiri</p>
            </div>
        </div>
    </div>
    <?php endif; ?>
