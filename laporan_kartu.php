...
// PERBAIKAN: Saldo awal periode dipaksa menjadi 0
...
if ($id_barang) {
    ...
    // Ambil transaksi untuk ditampilkan (dalam rentang periode)
...
@media print {
    ...
    #printable { visibility: visible !important; position: absolute; left: 0; top: 0; width: 100%; border: none !important; padding: 0 !important; }
    .table-custom th, .table-custom td { padding: 4px 8px !important; font-size: 9pt !important; border: 1px solid #000 !important; line-height: 1.1 !important; }
    .print-header { display: flex !important; justify-content: space-between; border-bottom: 2px solid #000; margin-bottom: 10px; padding-bottom: 10px; }
}
</style>
        ...
        <table class="table-custom">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="text-slate-700 uppercase text-xs"><?= htmlspecialchars($t['keterangan']); ?></td>
                    <td class="text-center text-emerald-600 font-black italic"><?= $masuk ? '+'.number_format($masuk) : '-' ?></td>
                    <td class="text-center text-rose-600 font-black italic"><?= $keluar ? '-'.number_format($keluar) : '-' ?></td>
                    <td class="text-center font-black text-slate-800 bg-slate-50/30"><?= number_format($running_saldo) ?></td>
                </tr>
                <?php endforeach; else: ?>
                <tr>
                    <td colspan="5" class="text-center py-20 text-slate-400 italic">Tidak ada mutasi pada periode ini.</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
